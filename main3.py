...
def add():
    ...
    tempo_de_exp = request.form['tempo_de_exp']
...
def salvar_no_banco(cv):
    ...
    cursor.execute('''
        INSERT INTO cv (nome, sobrenome, idade, formacao, habilidades, cidade, experiencia, anos, resultado_teste)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (cv['Nome'], cv['Sobrenome'], cv['Idade'], cv['Formacao'], cv['Habilidades'], cv['Cidade'], cv['Experiencia'], cv['Tempo'], 0))
